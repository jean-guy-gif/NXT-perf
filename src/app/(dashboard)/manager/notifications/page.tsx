...
// Local helper: map AppNotification type → icon + tint (cohérent avec /(dashboard)/notifications conseiller)
...
export default function ManagerNotificationsPage() {
  const { notifications, unresolvedCount, markAllAsRead, markAsResolved, loading } =
    useNotifications();
  const [filter, setFilter] = useState<FilterMode>("unresolved");

  // Zone 1 — alertes système computed (same source as bell counter)
  ...
  // Show zone 2 empty state only when DB has notifs (filter masks them) OR zone 1 is also empty
  const showZone2EmptyState =
    filtered.length === 0 && (notifications.length > 0 || localNotifs.length === 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground">
            Mes alertes en cours
          </h1>
          <p className="mt-2 text-muted-foreground">
            {unresolvedCount > 0
              ? `${unresolvedCount} non traitée${unresolvedCount > 1 ? "s" : ""}`
              : "Tout est traité"}
          </p>
        </div>
        {unresolvedCount > 0 && (
      ...
      {/* ═══ ZONE 1 — ALERTES SYSTÈME (computed manager via dispatcher) ═══ */}
        ...
        <section className="space-y-3">
          <div>
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Détectées automatiquement à partir de votre équipe. Mises à jour en
              continu.
            </p>
          </div>
      ...
      <div className="flex w-fit gap-1 rounded-lg bg-muted p-1">
        {(
          ...
          ] satisfies { key: FilterMode; label: string }[]
        ).map((f) => (
          <button
            key={f.key}
            type="button"
      ...
      {/* ═══ Empty state Zone 2 (intelligent) ═══ */}
      {showZone2EmptyState && (
