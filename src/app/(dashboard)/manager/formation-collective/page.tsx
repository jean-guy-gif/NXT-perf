...
export default function FormationCollectivePage() {
  ...
  const teamAnalysis = useMemo(() => {
    ...
    // Group by area with advisor names
    ...
    for (const { userName, diag } of diagnosticsByUser) {
      for (const rec of diag.recommendations) {
        if (!areaCounts[rec.area]) {
          areaCounts[rec.area] = { count: 0, totalGap: 0, label: rec.label, advisors: [] };
        }
    ...
    return Object.entries(areaCounts)
      .map(([area, data]) => ({
        ...
        advisors: data.advisors.sort((a, b) => a.priority - b.priority || b.gap - a.gap),
      }))
      .sort((a, b) => b.count - a.count);
  }, [conseillers, allResults, ratioConfigs]);

  // NXT Training mock data per advisor
  const nxtTrainingData = useMemo(() => {
    return conseillers.map((user) => {
      const results = allResults.find((r) => r.userId === user.id);
      if (!results) return { user, sessions: 0, hours: 0, progress: 0, weakAreas: 0 };
      const ratios = computeAllRatios(results, user.category, ratioConfigs);
      const weakCount = ratios.filter((r) => r.status !== "ok").length;
      // Simulated NXT data
      const sessions = Math.max(0, 7 - weakCount * 2 + Math.floor(Math.random() * 3));
      const hours = +(sessions * 0.75).toFixed(1);
      const progress = weakCount === 0 ? 100 : Math.min(95, Math.round(((7 - weakCount) / 7) * 100 + Math.random() * 15));
      return { user, sessions, hours, progress, weakAreas: weakCount };
    });
  }, [conseillers, allResults, ratioConfigs]);

  // Average ratios across team for bar chart
  ...
  const totalTeamSessions = nxtTrainingData.reduce((s, d) => s + d.sessions, 0);
  const totalTeamHours = nxtTrainingData.reduce((s, d) => s + d.hours, 0);
  const avgTeamProgress = nxtTrainingData.length > 0
    ? Math.round(nxtTrainingData.reduce((s, d) => s + d.progress, 0) / nxtTrainingData.length)
    : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Areas needing training with advisor names */}
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-muted-foreground">
                {area.count}/{conseillers.length} conseillers
              </span>
            </div>
            <ProgressBar
              value={(area.count / conseillers.length) * 100}
              status={
                area.count >= 3
                  ? "danger"
                  : area.count >= 2
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="mb-4 font-semibold text-foreground">
          Performance moyenne équipe par ratio (%)
        </h3>
        <BarChart
          data={avgRatioData}
          xKey="name"
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              NXT Training — Suivi équipe
            </h2>
            <p className="text-sm text-muted-foreground">
              Tableau de bord des performances d&apos;entraînement de votre équipe
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-5 text-center">
            <p className="text-xs text-muted-foreground">Heures d&apos;entraînement</p>
            <p className="mt-1 text-3xl font-bold text-indigo-500">
          ...
          <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-5 text-center">
            ...
            <p className={cn(
              ...
              avgTeamProgress >= 80 ? "text-green-500" : avgTeamProgress >= 60 ? "text-orange-500" : "text-red-500"
            )}>
              {avgTeamProgress}%
            </p>
          </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="space-y-3">
            {nxtTrainingData.map((d) => {
              const perfStatus = d.progress >= 80 ? "ok" : d.progress >= 60 ? "warning" : "danger" as const;
              return (
                <div
                  key={d.user.id}
                ...
                >
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {d.sessions} sessions · {d.hours}h · {d.weakAreas} point(s) faible(s)
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 sm:w-48">
                    ...
                    <span className={cn(
                      ...
                      perfStatus === "ok" ? "text-green-500" : perfStatus === "warning" ? "text-orange-500" : "text-red-500"
                    )}>
                      {d.progress}%
                    </span>
                  </div>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Gérez les parcours d&apos;entraînement de votre équipe et suivez leur progression en temps réel.
                </p>
              </div>
            </div>
            <button
