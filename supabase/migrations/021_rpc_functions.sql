-- ============================================================================
-- 021 — RPC Functions (server-side, SECURITY DEFINER)
-- ============================================================================

-- ── redeem_network_code ──────────────────────────────────────────────────────
-- Réseau admin only. Links an agency (organization) to a network.
...
BEGIN
  ...
  SELECT org_id INTO _user_org_id FROM profiles WHERE id = auth.uid();
...
-- ── redeem_agency_code ───────────────────────────────────────────────────────
-- Agent or manager joins an agency via code_agence.
...
BEGIN
  ...
  IF _org_id IS NULL THEN
    -- Fallback: try invite_code (legacy)
...
-- ── redeem_team_code ─────────────────────────────────────────────────────────
-- Agent joins a team via code_equipe.
...
-- ── redeem_coach_code ────────────────────────────────────────────────────────
-- Coachee attaches to a coach via coach_code.
...
BEGIN
  ...
  -- Upsert: if link exists and was revoked, reactivate
...
-- ── regenerate_agency_code ───────────────────────────────────────────────────
-- Directeur regenerates agency code. Existing members stay.
...
BEGIN
  -- Check caller is director of this agency
  SELECT EXISTS (
    SELECT 1 FROM agency_directors
    WHERE director_user_id = auth.uid() AND agency_id = p_org_id
  ) INTO _is_director;

  -- Also allow if caller is in this org as directeur
...
-- Manager regenerates team code. Existing members stay.
...
BEGIN
  SELECT EXISTS (
    SELECT 1 FROM teams WHERE id = p_team_id AND manager_id = auth.uid()
  ) INTO _is_manager;
...
-- ── revoke_coaching_link ─────────────────────────────────────────────────────
-- Coach or coachee can soft-revoke a link.
...
-- ── upgrade_role ─────────────────────────────────────────────────────────────
-- Upgrades user role with history tracking.
